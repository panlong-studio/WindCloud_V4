...
graph LR
    ...
    subgraph Server [服务端]
        ...
        MainThread[main线程]
        style MainThread fill:#fff,stroke:#333
        
        TaskQueue[队列]
        style TaskQueue fill:#fff,stroke:#333,stroke-dasharray: 5 5

        SubThread1[子线程一<br/>专门和一个客户端进行通信]
        style SubThread1 fill:#fff,stroke:#333
        
        SubThread2[子线程二<br/>专门和一个客户端进行通信]
        style SubThread2 fill:#fff,stroke:#333
    end
    ...
    %% 连接与逻辑流
    Client_ZS -- connect连接 --> AcceptQueue
    Client_LS -- connect连接 --> AcceptQueue
    
    AcceptQueue --> MainThread
    MainThread -- 获得连接<br/>放入队列 --> TaskQueue
    
    TaskQueue -- 获取 --> SubThread1
    TaskQueue -- 获取 --> SubThread2

    %% 独占式通信路径
    SubThread1 <== 通信 ==> Client_ZS
    SubThread2 <== 通信 ==> Client_LS
    ...
    %% 问题注释
    note[<b>上面模式中问题：</b><br/>1. 产生饥饿问题<br/>2. 或则连接浪费<br/>3. 如果客户端或者服务器在上传文件，<br/>只能等到文件传输完毕才能做别操作，操作不友好]
    note -.-> Server
    style note fill:#fff,stroke:#f66,stroke-dasharray: 5 5,color:#f66
```
...
graph LR
    ...
    subgraph Clients [客户端区域]
        direction TB
        subgraph Client_ZS [客户端: zs]
            ZS_Main[建立连接<br/>普通命令通信]
            ZS_Sub[只处理文件<br/>上传和下载]
            ZS_Main -- 文件上传和下载:<br/>交给子线程 --> ZS_Sub
        end

        subgraph Client_LS [客户端: ls]
            LS_Main[建立连接<br/>普通命令通信]
            LS_Sub[只处理文件<br/>上传和下载]
            LS_Main -- 文件上传和下载:<br/>交给子线程 --> LS_Sub
        end
    end
    ...
    subgraph Server [服务端]
        ...
        MainThread[main线程]
        style MainThread fill:#f9f,stroke:#333,stroke-width:2px
        
        LogicNote[代码逻辑:<br/>普通命令处理<br/>文件上传和下载交给子线程]
        
        TaskQueue[队列]
        style TaskQueue fill:#fff,stroke:#333,stroke-dasharray: 5 5

        SubThread1[子线程一<br/>只处理文件上传和下载]
        style SubThread1 fill:#dfd,stroke:#000
        
        SubThread2[子线程二<br/>只处理文件上传和下载]
        style SubThread2 fill:#dfd,stroke:#000
    end
    ...
    AcceptQueue --> MainThread
    MainThread -.-> LogicNote
    
    MainThread -- 文件上传和下载行为<br/>放入队列 --> TaskQueue
    
    TaskQueue -- 获取 --> SubThread1
    TaskQueue -- 获取 --> SubThread2

    %% 文件传输双向路径
    ZS_Sub <== 文件的上传和下载 ==> SubThread1
    LS_Sub <== 文件的上传和下载 ==> SubThread2
    ...
    %% 样式调整
    linkStyle 8,9 stroke:#2ca02c,stroke-width:2px;
```

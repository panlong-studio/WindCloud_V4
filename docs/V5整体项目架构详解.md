...
虽然仓库目录没有完全按“传统三层架构”命名，但从职责上已经可以清楚映射成下面几层：
...
因此第五期的数据源系统不是“额外手工建表”，而是服务端启动时自动保证存在。
...
#### B. 现在“秒传成功”不仅要求数据库有相同 hash，还要求真实文件可用

也就是说，服务端会先确认：

1. `files` 里存在同 hash 记录
2. `test/server_files/<sha256>` 这份真实文件当前确实存在且完整
...
#### C. 当前“多点下载”是基于多个完整数据源服务器，而不是单机多文件块表

也就是说，当前服务端并不会保存“某个文件只存了一半”的块信息。
...
DAO 层只负责执行 SQL，不负责组织“用户名重复时提示什么”这种业务语义。
...
- 根目录 `/` 在当前代码里约定为 `current_dir_id = 0`
- 它不是 `paths` 表里真的存在的一条普通记录

所以很多 `dao_vfs.c` 的实现都会把根目录作为特判处理。
...
1. `dao_file_source_upsert()`
   - 记录某个 `file_id` 当前可从哪台服务器下载
2. `dao_file_source_list()`
   - 查询某个 `file_id` 当前有哪些可用数据源
...
erDiagram
    ...
    files ||--o{ file_sources : served_by
...
1. 先用 `paths` 判断“当前目录下有没有 `report.pdf`”
2. 再从 `paths.file_id` 找到 `files.id`
3. 再通过 `files.id` 去 `file_sources` 查询当前有哪些服务器持有这份真实文件
...
当前客户端不再只是“拿一条 socket 发命令”的简单程序。
...
当前项目的重要协议包已经不只是一种“小火车”，而是按职责拆成了多类固定结构体。
...
> 把“结构体完整收发”这件事统一收口，避免业务层自己反复写 `send/recv` 拼接逻辑
